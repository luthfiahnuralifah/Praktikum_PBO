...
interface Notifikasi {
    void kirimNotifikasi(String pesan); // Method untuk mengirim notifikasi
    void kirimLaporan(String laporan); // Method untuk mengirim laporan
}
...
class Reseller extends Orang implements Notifikasi {
    ...
    public void kirimNotifikasi() {
        System.out.println("Notifikasi dikirim ke Reseller: " + getNama() + " melalui email: " + email);
    }

    // Implementasi method dari interface Notifikasi
    @Override
    public void kirimNotifikasi(String pesan) {
        System.out.println("Notifikasi ke Reseller " + getNama() + ": " + pesan);
    }

    @Override
    public void kirimLaporan(String laporan) {
        System.out.println("Laporan untuk Reseller " + getNama() + ": " + laporan);
    }
...
class Pelanggan extends Orang implements Notifikasi {
    ...
    @Override
    public void kirimNotifikasi() {
        System.out.println("Notifikasi dikirim ke Pelanggan: " + getNama() + " melalui SMS: " + telepon);
    }

    // Implementasi method dari interface Notifikasi
    @Override
    public void kirimNotifikasi(String pesan) {
        System.out.println("Notifikasi ke Pelanggan " + getNama() + ": " + pesan);
    }

    @Override
    public void kirimLaporan(String laporan) {
        System.out.println("Laporan untuk Pelanggan " + getNama() + ": " + laporan);
    }
...
final class Menu {  // Menambahkan keyword final pada class Menu
    ...
    // Menambahkan keyword final pada method toString
    @Override
...
public class JasaTitipApp {
    ...
    // Variable static baru untuk tracking statistik
    private static int totalPesanan = 0;
    private static double totalPendapatan = 0;

    // Method static baru untuk menambahkan pendapatan
    public static void tambahPendapatan(double jumlah) {
        totalPendapatan += jumlah;
        totalPesanan++;
    }

    // Method static untuk mendapatkan statistik
    public static String getStatistik() {
        return "Total pesanan: " + totalPesanan + ", Total pendapatan: Rp " + totalPendapatan;
    }
        ...
        while (running) {
            ...
            try {
                switch (pilihan) {
                    ...
                    case 6 -> lihatStatistik(); // Menu baru untuk melihat statistik
                    case 7 -> {
                        System.out.println("Terima kasih telah menggunakan aplikasi JasaTitip!");
                        scanner.close();
                        running = false;
                    }
                    default -> System.out.println("Pilihan tidak tersedia. Silakan pilih 1-7.");
                }
            } catch (Exception e) {
    ...
    static void tampilkanMenuUtama() {
        ...
        System.out.println("6. Lihat Statistik"); // Menu baru
    ...
    static void tambahReseller() {
        try {
            ...
            Reseller newReseller = new Reseller(nextId++, nama, alamat, telepon, email);
            resellerList.add(newReseller);
            
            // Menggunakan interface method
            newReseller.kirimNotifikasi("Selamat bergabung sebagai reseller Mie Gacoan!");
            newReseller.kirimLaporan("Akun anda telah dibuat pada " + new Date().toString());
    ...
    static void tambahPelanggan() {
        try {
            ...
            Pelanggan newPelanggan = new Pelanggan(nextId++, nama, alamat, telepon, email);
            pelangganList.add(newPelanggan);
            
            // Menggunakan interface method
            newPelanggan.kirimNotifikasi("Selamat bergabung sebagai pelanggan Mie Gacoan!");
            newPelanggan.kirimLaporan("Akun anda telah dibuat pada " + new Date().toString());
    ...
    static void buatPesanan() {
        try {
            ...
            // Menambahkan data pendapatan (static)
            tambahPendapatan(menuDipilih.getHarga() * jumlah);
            ...
            String pesanPelanggan = "Pesanan " + menuDipilih.getNama() + " sejumlah " + jumlah + " sedang " + status;
            String pesanReseller = "Ada pesanan baru untuk diproses: " + menuDipilih.getNama() + " sejumlah " + jumlah;
            
            pelangganDipilih.kirimNotifikasi(pesanPelanggan);
            resellerDipilih.kirimNotifikasi(pesanReseller);
            
            String laporanPelanggan = "Total biaya: Rp " + (menuDipilih.getHarga() * jumlah);
            pelangganDipilih.kirimLaporan(laporanPelanggan);
